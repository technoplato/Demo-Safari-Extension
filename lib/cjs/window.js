...
class RealGhost {
    ...
    signAllTransactions(transactions) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const base64Transactions = transactions.map((transaction) => {
                return {
                    transaction: buffer_1.Buffer.from(transaction.serialize()).toString('base64')
                };
            });
            window.postMessage({
                source: 'wallet-adapter-event',
            ...
            const signedTransactions = this.transactionBytesArray.map((transactionBytes) => {
                const transactionBuffer = buffer_1.Buffer.from(transactionBytes, 'base64');
                const signedLegacyTransaction = web3_js_1.Transaction.from(transactionBuffer);
